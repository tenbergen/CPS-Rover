...
class InfoUpdate:
    ...
    hedge = MarvelmindHedge(tty = "/dev/ttyACM0", adr = 5, debug = False) # establishing the hedge. The adr may be modified depending on the adr listed on your map.
    position = None
    otherPos = None
    onEdge = False
    ...
    def getUpdatedPosition(self):
        if (self.hedge.positionUpdated):
            ...
            self.position.append(self.onEdge)
