...
def not_in_polygon(s1, s2):
    ...
    while not is_inside_polygon(points = s1, p = (position[1], position[2])):
        ...
        if possible_collision():
            #check if the other rover is in front. if in front stop, else move forward
            if (otherPos[6] == True):
                collision_action_when_not_in_polygon(s2)
            elif start == False:
                gpg.stop()
                break
            else:
...
# check for possible collsion with stationary beacons
...
def collision_action(s2):
    ...
    if (xDis < box_len) and (yDis < box_len):
        gpg.stop()
        sleep(3)
        r1 = random.randint(150,180)
        r2 = random.randint(-180,-150)
...
#collision action that handles possible collision with stationary beacons
def collision_action2(SBpos, s2):
    SBposition = SBpos
    gpg.stop()
    sleep(3)
...
def collision_action_when_not_in_polygon(s2):
    xDis = abs(otherPos[1] - position[1])
    yDis = abs(otherPos[2] - position[2])
    box_len = 0.6
    cantMove = False
...
def getSBeaconPos():
    ...
    o = otherPos
    b10 = (p[1], p[2])
    b11 = calculate_pair_beacon_pos(p)
    b5 = (o[1], o[2])
    b10Dis = None
    b11Dis = None
    b5Dis = None
    ...
    while len(arrayOfSB) > 0:
        dis = iu.getDistances()
        if dis[0] == 10 and (10 in arrayOfSB):
            b10Dis = dis
            arrayOfSB.remove(10)
        elif dis[0] == 11 and (11 in arrayOfSB):
            b11Dis = dis
            arrayOfSB.remove(11)
        elif dis[0] == 5 and (5 in arrayOfSB):
            b5Dis = dis
            arrayOfSB.remove(5)
            
    i = 2
    while i < len(b10Dis):
        p = calculate_point(b10, b11, b5, b10Dis[i], b11Dis[i], b5Dis[i])
        midBound.append(p[0])
        outerBound.append(p[1])
        innerBound.append(p[2])
        i += 2
    
    MBCentroid = centroid(midBound)
    OBCentroid = centroid(outerBound)
    IBCentroid = centroid(innerBound)
    
    MBandOBxDif = OBCentroid[0] - MBCentroid[0]
    MBandOByDif = OBCentroid[1] - MBCentroid[1]
    IBandMBxDif = MBCentroid[0] - IBCentroid[0]
    IBandMByDif = MBCentroid[1] - IBCentroid[1]
...
def movement(boundaries):
    gpg.set_speed(150) # random.randint(100,150)
    while start:
        s1 = boundaries[1]
        s2 = boundaries[0]
        ...
        if possible_collision():
            ...
            collision_action2(collisionCond2, s2)
        elif (distance < 15):
            collision_action3()
        elif not is_inside_polygon(points = s2, p = (position[1], position[2])):
            not_in_polygon(s1, s2)
